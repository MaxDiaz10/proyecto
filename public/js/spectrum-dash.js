...
var spetsnaz = {
    dashboard: function () {
        if (methods.eclass("dashboard-box") == null) return;

        setInterval(function () {
            moonlightHelper.get("/api/balancers", {}, (response, status) => {
                if (response == null) return;

                //methods.eclass("balancer-list").innerHTML = response; No he tenido tiempo
            });
            ...
            moonlightHelper.get("/api/dashboard", {}, (response, status) => {
                ...
                methods.elem("request-graphic").classList = spetsnaz.progress(
                    "request",
                    perform.RequestGraphic
                );
                methods.elem("attack-graphic").classList = spetsnaz.progress(
                    "attack",
                    perform.AttackGraphic
                );
                methods.elem("flood-graphic").classList = spetsnaz.progress(
                    "flood",
                    perform.FloodGraphic
                );

                methods.sel("s-requests-today").innerHTML =
                    perform.TodayRequests;
                ...
                methods.sel("balancers-count").innerHTML =
                    perform.BalancerCount;
                methods.sel("balancers-connections").innerHTML =
                    perform.BalancerConnections;
            });
        }, 10000);
    },
    bans: function () {
        if (methods.eclass("list-bans") == null) return;

        document.querySelectorAll(".list-bans span").forEach(function (button) {
            button.addEventListener("click", function (e) {
                moonlightHelper.post(
                    "/api/delete",
                    { ban: this.getAttribute("id").replace("ban-", "") },
                    (response, status) => {
                        console.log(response);
                        spetsnaz.notification(
                            "Has eliminado un objetivo de la lista. Los cambios pueden tardar en aparecer hasta 30 segundos."
                        );
    ...
    progress: function (graphic, value) {
        if (graphic == null || value == null) return;
